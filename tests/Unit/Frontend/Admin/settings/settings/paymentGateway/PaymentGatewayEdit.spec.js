...
describe('PaymentGatewayEdit.vue', () => {
    ...
    beforeEach(() => {
        ...
        global.mockHttp.onGet(/\/get-stripe-settings/).reply(200, {
    ...
    it('fetches payment gateway list on mount', async () => {
        await flushPromises()
        expect(global.mockHttp.history.get.some(r => r.url.includes('payment-gateway-list'))).toBeTruthy()
    })

    it('fetches gateway settings for stripe on mount', async () => {
        await flushPromises()
        expect(global.mockHttp.history.get.some(r => r.url.includes('get-stripe-settings'))).toBeTruthy()
    })

    it('calls save endpoint on form submit', async () => {
        global.mockHttp.onGet(/\/update-api-key\/payment-gateway\/stripe/).reply(200, { data: {} })
        await flushPromises()
        const { validateForm } = require('@/helpers/formUtils.js')
        validateForm.mockResolvedValueOnce(true)
        wrapper.vm.save()
        await flushPromises()
        expect(global.mockHttp.history.get.some(r => r.url.includes('update-api-key/payment-gateway/stripe'))).toBeTruthy()
    })
})
