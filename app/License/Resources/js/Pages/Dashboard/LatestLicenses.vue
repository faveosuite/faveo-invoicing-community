<template>
    <div class="card card-light">
        <div class="card-header">
            <h4 class="card-title">{{ lang('latest_licenses') }}</h4>
...
const emit = defineEmits(['latestLicenses'])

const columns = ['license_code', 'product', 'license_date', 'license_status']
const counter = ref(0)
...
function getData() {
    ...
    axios.get(baseUrl + '/api/admin/dashboarddropdown').then((res) => {
        ...
        const { data } = res.data
        if (data) {
            emit('latestLicenses', 'latestLicenses', data)
        }
    }).catch(() => {
        loading.value = false
    })
