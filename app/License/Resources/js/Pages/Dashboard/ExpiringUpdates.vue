<template>
    <div class="card card-light">
        <div class="card-header">
            <h4 class="card-title">{{ lang('expiring_updates') }}</h4>
...
const emit = defineEmits(['expiringUpdates'])

const columns = ['license_code', 'product', 'license_date', 'license_updates_date', 'license_status']
const counter = ref(0)
...
const options = reactive({
    ...
    headings: {
        ...
        license_updates_date: lang('updates'),
...
function getData() {
    ...
    axios.get(baseUrl + '/api/admin/dashboarddropdown').then((res) => {
        ...
        const { data } = res.data
        if (data) {
            emit('expiringUpdates', 'expiringUpdates', data)
        }
    }).catch(() => {
        loading.value = false
    })
