...
class LicenseController extends Controller
{
    ...
    public function edit($license_id)
    {
        ...
        $productName = collect([(object) ['name' => optional($license->product)->name, 'id' => $license->id]]);
        $clientName = collect([(object) ['full_name' => trim(optional($license->user)->first_name.' '.optional($license->user)->last_name).' <'.optional($license->user)->email.'>', 'id' => $license->user_id]]);
    ...
    public function syncTheCreationOfLicense(Request $request)
    {
        try {
            ...
            $inputOptions = json_decode($request->input('options', '[]'), true);
    ...
    public function licenseInfo(Request $request)
    {
        ...
        $addons = $license->addonProducts->map(function ($product) {
            return [
                'id' => $product->id,
    ...
    public function individualLicenseInfo(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $licenseOptions = $license->licenseOptions->map(function (LicenseOption $option) use ($license) {
            return [
                'license_code' => $license->license_code,
                'id' => $option->id,
    ...
    {
        $licenseCodes = collect(json_decode($request->input('license_code'), true));
        $licenses = License::whereIn('license_code', $licenseCodes)
            ->where(function ($q) {
                $q->where('license_expire_date', '>', Carbon::now())
