...
class ProductsViewController extends Controller
{
    ...
    public function getProductInstallations(Request $request, $id)
    {
        ...
        $productInstallations = DB::table('installations')
            ->select('installations.id','installations.id','installations.user_id as client_id','installations.license_code','installations.installation_domain','installations.installation_ip','installations.installation_date','installations.installation_status')
            ...
            ->selectRaw('installations.id, installations.id, installations.user_id as client_id, installations.license_code, installations.installation_domain, installations.installation_ip, installations.installation_date, installations.installation_status, users.email as client_email, licenses.id as license_id, licenses.license_code as license_code_detail')
    ...
    public function getProductLicenses(Request $request, $id){
        ...
        $productLicenses = DB::table('licenses')
            ->selectRaw('licenses.id as license_id, licenses.id, licenses.user_id as client_id, licenses.license_code, licenses.license_order_number, licenses.license_date, licenses.license_expire_date, licenses.license_updates_date, licenses.license_support_date, licenses.license_status, users.email as client_email')
    ...
    public function getProductVersions(Request $request, $productId)
    {
        ...
        $productVersions = ProductVersion::where('id', $product)
