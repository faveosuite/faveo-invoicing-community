...
class TaxController extends Controller
{
    ...
    public function editTaxApi($id)
    {
        try {
            ...
            $options = $this->tax_option->find(1);
            $txClass = $this->tax_class->find($tax->tax_classes_id);
            $taxClassName = $txClass->name ?? null;
     ...
     * @return type
     */
    public function getState(Request $request, $countryCode)
    {
        try {
            $states = State::where('country_code_char2', $countryCode)
                ->orderBy('state_subdivision_name', 'asc')
                ->get(['state_subdivision_code', 'state_subdivision_name']);

            if ($states->isEmpty()) {
                return errorResponse(__('message.no-record-found'), 404);
            }
