...
class DeployController extends BaseClientController
{
    ...
    public function getVersions($orderId)
    {
        try {
            ...
            if (! authorizeOwnership($order->client)) {
                return response()->json(['error' => __('message.unauthorized_action')], 403);
            }
    ...
    private function stepVerify(Request $request, $credential, string $deployPath): JsonResponse
    {
        ...
        if (! $ssh->login($request->username, $credential)) {
            $errors = implode(' | ', $ssh->getErrors() ?: ['Authentication failed']);
            return response()->json(['status' => 'false', 'message' => 'SSH authentication failed: ' . $errors]);
        }
        ...
        if ($request->deploy_mode === 'extract_only') {
            ...
            if (str_contains($result, 'failed')) {
                return response()->json(['status' => 'false', 'message' => "Could not create deploy path '{$deployPath}'. Check permissions."]);
            }
        }

        return response()->json(['status' => 'success', 'message' => 'SSH connection verified.']);
    }
    ...
    private function stepInstall(Request $request, $credential): JsonResponse
    {
        ...
        $installScript = 'https://github.com/ladybirdweb/faveo-server-images/raw/master/installation-scripts/FaveoInstallationScripts/install.sh';
        ...
        if (str_contains($output, 'error') || str_contains($output, 'failed')) {
            return response()->json([
                'status'  => 'false',
                'message' => 'Installation script encountered errors.',
                'output'  => $output,
            ]);
        }
    ...
    private function stepUpload(Request $request, $credential): JsonResponse
    {
        ...
        if (! authorizeOwnership($order->client)) {
            return response()->json(['status' => 'false', 'message' => __('message.unauthorized_action')]);
        }
        ...
        if (empty($upload->file)) {
            return response()->json(['status' => 'false', 'message' => 'No file attached to this version.']);
        }

        $filePath = 'products/' . $upload->file;

        if (! Attach::exists($filePath)) {
            return response()->json(['status' => 'false', 'message' => 'Product file not found in storage.']);
        }
        ...
        if (! $sftp->login($request->username, $credential)) {
            @unlink($tmpFile);
            return response()->json(['status' => 'false', 'message' => 'SFTP connection failed.']);
        }

        if (! $sftp->put($remotePath, $tmpFile, SFTP::SOURCE_LOCAL_FILE)) {
            @unlink($tmpFile);
            return response()->json(['status' => 'false', 'message' => 'Failed to upload file to the remote server.']);
        }
    ...
    private function stepExtract(Request $request, $credential, string $deployPath): JsonResponse
    {
        ...
        if (str_contains($output, 'Permission denied')) {
            return response()->json([
                'status'  => 'false',
                'message' => "Permission denied on '{$deployPath}'. Provide a sudo password or use a user with write access.",
                'output'  => $output,
            ]);
        }
