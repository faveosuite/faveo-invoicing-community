...
# label names
# ---------------------------------------------------------------------------
# This pipeline's labels ("Requires Functionality Review", "Functionality
# Correction", "QA: Round 1 Testing Approved", "Code Approved", …) carry no
# emoji shortcode in this repo today, but exact string matching against a label
# name is still the wrong default: a rename, a re-casing, or someone appending a
# shortcode later turns a configured label name into a permanent, silent miss —
# which on the Stage 3 trigger means skipping every PR and saying nothing.
#
...
# folded, and answer with the repo's exact spelling. Cheap insurance against a
# label edit nobody remembers to update here.
#
...
# Resolved first so a configured name still removes the repo's real label if it
# has since been renamed or had a shortcode added (e.g. "Functionality
# Correction" removing "Functionality Correction :facepunch:").
gh_remove_label() {
...
# an emoji shortcode added or removed. A gate that goes silent because someone
# edited a label is worse than one that is slightly generous.
gh_has_label() {
...
#
# first:100 is the GraphQL page maximum, and no release PR exceeds it in practice.
# It was 20, which silently truncated: GitHub returns these in ASCENDING issue
# number, not the order they appear in the PR body, so a release PR linking 31
# issues lost its 11 NEWEST ones — the ones most likely to be what that build is
# for. The round still executes a single issue, but the report's passed-over list
# is what tells a reader which issues went unexamined, and a list that silently
# stops at 20 makes that report wrong rather than incomplete.
gh_pr_linked_issues() {
  ...
  query=$(jq -n --arg o "$owner" --arg r "$repo_name" --argjson pr "$pr" '{
    query: "query($o:String!,$r:String!,$pr:Int!){repository(owner:$o,name:$r){pullRequest(number:$pr){closingIssuesReferences(first:100){nodes{number}}}}}",
