...
Every fact below was confirmed against the live `ekXp` project or by reading
...
- `ekXp` holds ~8,200 cases. Page size is **20**, so ~411 pages.
...
1. **`search_test_cases` and `search_modules` only search page 1.**
   ...
   filters client-side. On `ekXp` that searches 20 of 8,200 cases — 0.24% — and returns
...
3. **Bulk creation caps at 10 cases**, each needing **≥4 steps**, each step needing a
   **non-empty expected result**. The existing `[TC_SB_*]` cases in `ekXp` violate the last
...
- **`getAllModules` returns `section_name` / `section_key`, not `module_name` /
  ...
  `POST /testCase/steps` uses for `sectionKey`. Verified live on `ekXp`
...
- **Module names are matched case-insensitively by QA Touch, and `getAllModules` is
  ...
  `ekXp` holds **`Form builder`** (lowercase b, key `v1qgp`, page 4 of 17). An exact
...
- **A module key the module listing hides can still be recovered from a test case.**
  ...
  requests on `ekXp` (page size fixed at 20), so it is a last resort — but it turns
...
- **The API's case code is not what the UI shows.** `getAllTestCases` reports
  `case_code: "TR8967"`; the QA Touch UI shows the same case with the project's own
  code prefix plus the number instead (e.g. `FH-8967` in the project this was
  verified against — every project picks its own prefix at setup, so this app's
  `ekXp` prefix, if it has one, is not yet verified). Verified across the library:
  even 2019-era cases created by hand in the UI come back as `TR0064`, so `TR` is
  the API's rendering, not an artefact of how a case was created. The number is the
  shared part. `GET /projects` does NOT expose the prefix (only name, key, url,
  estimate, status), so it is configured as `QATOUCH_CASE_PREFIX` — left empty by
  default until someone confirms `ekXp`'s prefix, which safely publishes the raw
  `TR####` code instead of guessing wrong. Publish the UI form to people and keep
  `TR####` in the marker — that is the form the results endpoint accepts.
...
nothing for cases. All 27 runs in `ekXp` are `"Type": "specific"`, i.e. created in
...
`ekXp`, which is why that round concluded no such endpoint existed. It is in the
...
calls these "releases"). There is deliberately **no fallback**: all 16 in `ekXp`
...
other.** GitHub carries `Invoicing v9.4.3.8.RC.1`; QA Touch carries
`Release 9.4.3.8 RC1`, `Release 9.2.1 Official`, `Faveo Invoicing release 7.1.6`,
even `Relaase 9.1.0 RC`. So `qt_canon_milestone` reduces both to
`<version>|<qualifier>` — product words (the product name, `faveo`, `release`, that typo,
...
|---|---|---|
| `Invoicing v9.4.1.2` | `9.4.1.2` | `Release 9.4.1.2` |
| `Invoicing v9.2.1` | `9.2.1` | `Release 9.2.1 Official` |
| `Invoicing v9.2.1.RC.1` | `9.2.1` + `rc1` | `Release 9.2.1 RC1` |
| `Invoicing v9.4.1.2.RC.1` | `9.4.1.2` + `rc1` | **nothing — refuses** |
...
3. ~~`POST /testRun`~~ — **closed, and not a judgement call any more.** Measured
   against `ekXp` on 2026-08-21, with `projectKey`, `testRun`, `assignTo` and
