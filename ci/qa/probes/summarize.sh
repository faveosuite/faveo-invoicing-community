...
# absence of evidence: if a login never succeeds, the auth-boundary checks that
# depend on it each emit a skip, there are zero blocking failures, and a naive
# rule would sign the round off having never actually exercised the boundary.
# "We could not check" must never render as "we checked and it was fine".
#
# Warnings never decide the verdict. A first round that fails a PR for a
# pre-existing hardening gap or a missing header fails every PR equally, and a
# gate that is always red is a gate nobody reads. Blocking findings are the
# ones with no judgement in them: a served .env, a stack trace in a response,
# an admin endpoint answering anonymously.
