...
# pr_marker_json <pr> <head-sha> -> the marker JSON for that commit, or nothing
#
# The whole object, for callers that need more than the issue list — publishReport
# unions the issues AND sums the counts, because the PR carries one marker-keyed
# comment that publishing replaces in place.
pr_marker_json() {
  gh_comments "$1" | jq -r '.[].body' \
    | grep -o "${PR_MARKER_OPEN}.*-->" \
    | sed "s|^${PR_MARKER_OPEN}||; s| -->$||" \
    | jq -c --arg sha "$2" 'select(.sha == $sha)' 2>/dev/null \
    | tail -1
}

# pr_marker_issues <pr> <head-sha> -> one issue number per line, ascending
#
# Which linked issues a previous round actually FINISHED for this commit. A round
# covers every qualifying linked issue and can stop part-way through them — on a
# per-issue timeout, on a usage limit, or because the build ran out of clock — so
# "already reported" has to be per issue or the unrun remainder is suppressed for
# the life of that commit, which is the one failure mode here that hides itself.
#
# Empty for a v1 marker, which carries no issue list. That is not "nothing ran": v1
# means the whole round was reported, and pr_marker_exists still covers it.
pr_marker_issues() {
  gh_comments "$1" | jq -r '.[].body' \
    | grep -o "${PR_MARKER_OPEN}.*-->" \
    | sed "s|^${PR_MARKER_OPEN}||; s| -->$||" \
    | jq -r --arg sha "$2" 'select(.sha == $sha) | .issues[]? | tostring' 2>/dev/null \
    | sort -un
}

# pr_marker_build <head-sha> <run-key> <passed> <failed> <blocked> [issues-json]
#
# v2 adds `issues`: the issue numbers this round executed. Counts are the sum across
# them. Readers of v1 keep working — every v1 field is still present and means the
# same thing.
pr_marker_build() {
  local json
  json=$(jq -cn --arg sha "$1" --arg run "$2" \
    ...
    --argjson issues "${6:-[]}" \
    '{v:2, sha:$sha, run:$run, passed:$passed, failed:$failed, blocked:$blocked, issues:$issues}')
