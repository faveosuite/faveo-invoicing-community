...
#   unreadable  fetched, but not usable here (.docx, .xlsx, .zip, video). A video
#               additionally has frames sampled from it where ffmpeg is available;
#               those are recorded as readable images of their own, and the video
#               itself stays unreadable so nobody assumes it was watched.
...
# video_frames <video-path> <url> <n> -> 0 if it recorded readable frames
#
# On a bug report the screen recording IS the requirement, and a model cannot watch
# one — so before this, a `.mp4` was classified `unreadable` and the cases were
# written from prose alone. Issue #15987 is the example: the recording held the
# search term, the result tab and the number of rows on screen, and all three came
# back as "not covered" in the authored set.
#
# Sampled frames are not the recording, but for a UI defect they carry most of it:
# the screen before, the action, and the screen after. Anything in the audio or
# between samples is still missing, so needs_more_info stays set either way and the
# manifest says these are frames, never that the video was read.
#
# ffmpeg is optional. Without it the classification is exactly what it was.
video_frames() {
  local src="$1" url="$2" idx="$3" got=0 f base
  command -v ffmpeg >/dev/null 2>&1 || return 1

  base="${out_dir}/video-${idx}-frame"
  # One frame every 4 seconds, 12 at most: enough to cover a two-minute capture
  # without spending the agent's whole context on near-identical screenshots.
  ffmpeg -nostdin -loglevel error -i "$src" \
         -vf "fps=1/${QA_VIDEO_FRAME_SECONDS:-4},scale=1280:-2" \
         -frames:v "${QA_VIDEO_MAX_FRAMES:-12}" -q:v 4 \
         "${base}-%02d.jpg" </dev/null >/dev/null 2>&1 || return 1

  for f in "${base}"-*.jpg; do
    [[ -f "$f" ]] || continue
    # A frame of a solid colour is a title card or a fade; it costs context and
    # shows nothing.
    [[ "$(wc -c < "$f")" -ge 8000 ]] || { rm -f "$f"; continue; }
    got=$(( got + 1 ))
    record "$url" image readable "$f" "frame ${got} of the screen recording, sampled every ${QA_VIDEO_FRAME_SECONDS:-4}s"
  done
...
web_page() {
  ...
  case "$sniffed" in
    ...
    video/*)
      mv -f "$raw" "${out_dir}/video-${n}"
      if video_frames "${out_dir}/video-${n}" "$url" "$n"; then
        # Still unreadable as a video: the frames are a sample, and the note says so
        # rather than letting a reader assume the recording was watched.
        record "$url" video unreadable "${out_dir}/video-${n}" \
          "$sniffed — not watchable, but frames were extracted and are listed above"
      else
        record "$url" video unreadable "${out_dir}/video-${n}" \
          "$sniffed — nobody here can watch it$(command -v ffmpeg >/dev/null 2>&1 || printf ' (ffmpeg is not installed on this node)')"
      fi ;;
