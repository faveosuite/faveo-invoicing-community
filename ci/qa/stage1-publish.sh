...
# part of the specification was never read. Not verified to exist in this repo yet —
# override QA_NEEDS_INFO_LABEL, or create the label, before relying on it.
...
# case library, with no marker recording which half.
...
if [[ -z "$module_key" ]]; then
  ...
  if [[ -z "$module_key" ]]; then
    ...
    # than a hunt through the whole tree.
...
# ---------------------------------------------------------------------------
# QA Touch renders a case as <project prefix>-<number> in its UI — e.g. AB-8967 —
# while the API reports the very same case as TR8967. Same case, same number, two
# renderings. Publishing the API form meant a tester read TR8967 on the issue,
# searched QA Touch for it, and found nothing.
#
...
# Unset (the default for this project — unverified, so left blank rather than
# guessed) means "show the API code", which is never wrong, only less convenient.
