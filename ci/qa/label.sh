...
# used to build these calls with curl in two places; the duplicate matters
# because a raw POST with a near-miss name (a rename, a re-casing, an emoji
# shortcode added later) CREATES a second, near-identical label instead of
# failing — see the "label names" section of gh-client.sh.
#
