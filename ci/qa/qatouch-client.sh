...
# Optional env:
#   QATOUCH_PROJECT    default project key (default: ekXp)
...
qt_project() { printf '%s' "${QATOUCH_PROJECT:-ekXp}"; }
...
# One page of the case library, cached for the life of a round when
# QT_PAGE_CACHE_DIR is set.
#
# The cache is what makes per-code lookups affordable. A marker's codes sit on the
# same one or two pages, so N codes re-fetch the same pages N times over a link that
# takes seconds per call — a 17-case marker spent 180s doing exactly that and was
# killed on the tool timeout before returning anything. The library does not change
# during a round, so re-reading it is pure waste.
#
# Written through a temp file and validated before it is kept: a truncated or error
# response cached as if it were a page would turn one bad call into a wrong answer
# for the rest of the round.
qt_cases_page() {
  local page="${1:-1}" cache tmp

  if [[ -z "${QT_PAGE_CACHE_DIR:-}" ]]; then
    qt_public_get "getAllTestCases/$(qt_project)?page=${page}"
    return $?
  fi

  mkdir -p "$QT_PAGE_CACHE_DIR" 2>/dev/null || true
  cache="${QT_PAGE_CACHE_DIR}/cases-$(qt_project)-${page}.json"
  ...
  tmp="${cache}.$$"
  if qt_public_get "getAllTestCases/$(qt_project)?page=${page}" > "$tmp" 2>/dev/null \
     && jq -e '.data' < "$tmp" >/dev/null 2>&1; then
    mv -f "$tmp" "$cache"
    cat "$cache"
    return 0
  fi
...
# led that round to conclude no such endpoint exists. Verified against a live
# project: HTTP 200 {"success":true,"data":[{"user_key":"0Dmw","name":"admin",
...
# be compared: GitHub carries "Invoicing v9.4.3.8.RC.1" and QA Touch carries
# "Release 9.4.3.8 RC1", and neither is a substring of the other.
#
#   Invoicing v9.4.3.8.RC.1       -> 9.4.3.8|rc1
#   Release 9.4.1.2               -> 9.4.1.2|
#   Invoicing v9.4.1.2            -> 9.4.1.2|
#   Release 9.2.1 RC1             -> 9.2.1|rc1
#   Release 9.2.1 Official        -> 9.2.1|       (an unqualified version, so equal
#                                                  to GitHub's "Invoicing v9.2.1")
#   Faveo Invoicing release 7.1.6 -> 7.1.6|
#   Relaase 9.1.0 RC              -> 9.1.0|rc1     (their typo, and a bare RC)
#
# Product words are noise and dropped: the product name, faveo, release, the
# misspelling already in the data, build, a "v" glued to a version — and
# "official", because an unqualified version IS the official release. That is
# what lets GitHub's "Invoicing v9.2.1" meet QA Touch's "Release 9.2.1 Official"
# while staying distinct from "Release 9.2.1 RC1". A project holding both
# "Release 9.2.1" and "Release 9.2.1
...
# on purpose ("Release 9.2.1 RC1" and "Release 9.2.1 Official" can both exist in
# the same project).
qt_canon_milestone() {
  printf '%s' "$1" | awk '
    {
      ...
      for (i = 1; i <= n; i++) {
        ...
        if (tok == "" || tok == "v" || tok == "invoicing" || tok == "faveo" \
...
#
# There is deliberately NO fallback. Every milestone in a project like this is a
# product release ("Release 9.4.1.2", "Faveo Invoicing release 7.1.6"), and quietly filing an
...
qt_default_milestone() {
  ...
  # and it is the only way "Invoicing v9.4.3.8.RC.1" can ever meet "Release 9.4.3.8 RC1".
  #
...
qt_module_key_by_name() {
  ...
  # names like "Create Package " and "Faveo Invoicing Community " are real.
  key=$(jq -r --arg n "$name" '
...
# finding a case body
# ---------------------------------------------------------------------------
# There is no fetch-one-case endpoint and search only sees page 1 (qatouch-facts.md
# trap 1), so a body has to be found by paging. This USED to scan backwards from the
# newest page for a bounded 15 pages, on the reasoning that Stage 1's cases are
# recent — and that stopped being true as the library grew. MeLq now holds 8,916
# cases over 446 pages, and a round's markers sit on pages 418-429: outside the
# window, every time. Every case in a round came back "not found — treat as Blocked",
# which is not merely a miss but a WRONG verdict, reported against cases that exist
# and are fully populated.
#
# Recency is the wrong index. getAllTestCases is ordered created_date ASCENDING and
# case codes increase with creation, so page number is monotonic in code — which
# makes the page findable by bisection in ~log2(446) ≈ 9 reads instead of 15 that
# cannot reach it. Verified against the library this was built for: TR9105-TR9124 on
# page 418, TR9645-TR9664 on page 445.

# qt_code_number <TR####> -> 9109
qt_code_number() { printf '%s' "${1//[!0-9]/}"; }

# qt_cases_locate_page <TR####> -> the page whose code range contains it
#
# Bisection over page number. Falls back to the last page when the wanted code is
# newer than anything in the library, which is what happens when a marker is read
# before its cases finish being written.
qt_cases_locate_page() {
  local code="$1" want lo=1 hi mid page_json first last_code
  want=$(qt_code_number "$code")
  [[ -n "$want" ]] || return 1

  hi=$(qt_cases_last_page)
  [[ -n "$hi" ]] || return 1

  while (( lo < hi )); do
    mid=$(( (lo + hi) / 2 ))
    page_json=$(qt_cases_page "$mid") || return 1

    # The page's own range, read from the data rather than assumed from the page
    # number: a page can be short, and the last page usually is.
    first=$(jq -r '[.data[]?.case_code] | map(gsub("[^0-9]";"") | tonumber) | min // empty' <<<"$page_json")
    last_code=$(jq -r '[.data[]?.case_code] | map(gsub("[^0-9]";"") | tonumber) | max // empty' <<<"$page_json")

    if [[ -z "$first" || -z "$last_code" ]]; then
      # An empty page tells us nothing about direction; stop rather than loop.
      break
    fi

    if (( want < first )); then
      hi=$(( mid - 1 )); (( hi < 1 )) && hi=1
    elif (( want > last_code )); then
      lo=$(( mid + 1 ))
    else
      printf '%s' "$mid"
      return 0
...
# qt_cases_by_codes <TR####,TR####,…> -> JSON array of full case objects
#
# One bisection plus a short forward sweep for the WHOLE marker, instead of an
# independent scan per code. A marker's codes are authored together and land on
# consecutive pages, so the sweep is normally one or two reads; the cache makes a
# later per-code call free.
#
# Bounded by QT_CASE_SWEEP_PAGES either side. Codes still missing after that are
# simply absent from the result — the caller reports them Blocked, and unlike the
# old scan that verdict is now true.
qt_cases_by_codes() {
  local wanted="$1" sweep="${QT_CASE_SWEEP_PAGES:-8}"
  [[ -n "$wanted" ]] || { printf '[]'; return 0; }

  local codes=() tok filter start last page found='[]' page_json hits
  IFS=',' read -r -a codes <<<"$wanted"
  filter=$(printf '%s\n' "${codes[@]}" | sed '/^$/d' | jq -R . | jq -sc .)

  start=$(qt_cases_locate_page "${codes[0]}") || return 1
  last=$(qt_cases_last_page)

  # Backwards first, then forwards: the located page is where the LOWEST code sits,
  # so the rest of the marker is at or after it.
  local lo=$(( start - 1 )); (( lo < 1 )) && lo=1
  local hi=$(( start + sweep )); (( hi > last )) && hi=$last

  for (( page = lo; page <= hi; page++ )); do
    page_json=$(qt_cases_page "$page") || continue
    hits=$(jq -c --argjson want "$filter" \
             '[.data[]? | select(.case_code as $c | $want | index($c))]' <<<"$page_json")
    found=$(jq -c --argjson a "$found" --argjson b "$hits" -n '$a + $b')
    if (( $(jq -r 'length' <<<"$found") >= ${#codes[@]} )); then
      break
    fi
  done
...
#
# Kept because the executor prompt calls it per code. It is now a bisection rather
# than a backwards scan, and with QT_PAGE_CACHE_DIR set the pages a previous code
# already read cost nothing.
qt_case_by_code() {
  local code="$1" page last hit sweep="${QT_CASE_SWEEP_PAGES:-8}" i

  page=$(qt_cases_locate_page "$code") || return 1
  last=$(qt_cases_last_page)

  # The located page, then its immediate neighbours: bisection lands exactly when
  # codes are strictly monotonic across pages, and a deleted or re-created case can
  # put a code one page off.
  for (( i = 0; i <= sweep; i++ )); do
    for candidate in $(( page + i )) $(( page - i )); do
      (( candidate < 1 || candidate > last )) && continue
      hit=$(qt_cases_page "$candidate" | jq -c --arg c "$code" '.data[]? | select(.case_code == $c)')
      if [[ -n "$hit" ]]; then
        printf '%s' "$hit"
        return 0
      fi
  ...
  printf 'qatouch: case %s not found near page %s (searched ±%s) — treat as Blocked, do not guess its steps\n' \
    "$code" "$page" "$sweep" >&2
  return 1
}
...
#   2b. any name in QA_RUN_NAME_FALLBACKS (pipe-separated), tried in order. A round
#      that executes several linked issues wants "First round — PR #<pr> (issue
#      #<issue>)" when someone made a run for that issue, and a single PR-level
#      run when they did not — because runs can only be made by hand in the UI
#      (POST /testRun is closed, see qatouch-facts.md), and one run per linked
#      issue is a chore nobody will keep up with on a PR that links thirty.
...
qt_resolve_run() {
  local name="$1" cases="${2:-}" description="${3:-}" key="" candidate tried=()
  ...
  # The preferred name, then each fallback in order. Read from a list rather than
  # two copies of the lookup so the "which names did we try" message below cannot
  # drift from what was actually attempted.
  while IFS= read -r candidate; do
    [[ -n "$candidate" ]] || continue
    tried+=("$candidate")
    if key=$(qt_run_key_by_name "$candidate" 2>/dev/null) && [[ -n "$key" ]]; then
      if [[ "$candidate" == "$name" ]]; then
        printf 'qatouch: reusing existing run "%s" -> %s\n' "$candidate" "$key" >&2
      else
        printf 'qatouch: no run named "%s" — falling back to "%s" -> %s\n' "$name" "$candidate" "$key" >&2
      fi
      printf '%s' "$key"
      return 0
    fi
    # Both printf's end in a newline: a final line without one makes `read` return
    # non-zero, and the loop then exits WITHOUT running the body for it — silently
    # dropping the last fallback, which is the only one in the common case.
  done < <(printf '%s\n' "$name"; printf '%s\n' "${QA_RUN_NAME_FALLBACKS:-}" | tr '|' '\n')

  if [[ "${QA_ALLOW_RUN_CREATE:-0}" != "1" ]]; then
    printf 'qatouch: no run found (tried: %s) and QA_ALLOW_RUN_CREATE is off — statuses will not be written\n' \
      "$(IFS='", "'; printf '"%s"' "${tried[0]}"; for ((i=1;i<${#tried[@]};i++)); do printf ', "%s"' "${tried[i]}"; done)" >&2
    return 1
  fi
