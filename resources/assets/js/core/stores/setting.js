...
export const useSettingStore = defineStore('setting', {
    ...
    getters: {
        getGeneralSettings: (state) => state.settings?.SETTING_ID ? {
        ...
        getAdvancedSettings: (state) => state.settings?.SETTING_ID ? {
            SETTING_ID: state.settings.SETTING_ID,
            API_STATUS: state.settings.API_STATUS,
            ENVATO_API_TOKEN: state.settings.ENVATO_API_TOKEN,
        } : null,
        getSecuritySettings: (state) => state.settings?.SETTING_ID ? {
            SETTING_ID: state.settings.SETTING_ID,
        ...
        getEmailSettings: (state) => state.settings?.SETTING_ID ? {
            SETTING_ID: state.settings.SETTING_ID,
        ...
        getCleanUpSettings: (state) => state.settings?.SETTING_ID ? {
            SETTING_ID: state.settings.SETTING_ID,
    ...
    actions: {
        async fetchSettings() {
            try {
                ...
                const res = await axios.get(`${baseUrl}/api/admin/viewSettings`)
                if (res?.data?.data?.[0]) {
